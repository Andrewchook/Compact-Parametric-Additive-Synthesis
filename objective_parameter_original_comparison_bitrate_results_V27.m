%% objective_parameter_original_comparison_results_V27.m
% Batch test script for thesis Results chapter original-vs-resynthesis quantitative tables.
%
% Purpose:
%   This script compares each V27 compact resynthesis directly against
%   its original audio recording. It also reports the compact parameter
%   storage needed for that same full model. No ablation comparisons
%   are run here.
%
...
%
% Main outputs:
%   objective_parameter_original_results_V27/tables/combined_quantitative_table.csv
%   objective_parameter_original_results_V27/tables/objective_metric_table.csv
%   objective_parameter_original_results_V27/tables/parameter_reduction_table.csv
%   objective_parameter_original_results_V27/tables/bitrate_comparison_table.csv
%   objective_parameter_original_results_V27/tables/bitrate_summary_table.csv
%   objective_parameter_original_results_V27/tables/literature_bitrate_template.csv
%   objective_parameter_original_results_V27/tables/harmonic_error_table.csv
%   objective_parameter_original_results_V27/tables/summary_by_family.csv
%   objective_parameter_original_results_V27/tables/failure_table.csv
%
% Notes:
...
%   - The stored-parameter counts are computed from the full compact V27
%     parameter struct used to synthesize the comparison audio.

clear; close all; clc;

%% ---------------- User settings ----------------
cfg = struct();
cfg.resultsDir = fullfile(pwd, 'objective_parameter_original_results_V27');
cfg.numHarmonics = 15;

% This script intentionally runs only the full compact V27 model.
% Each output row compares original audio against one resynthesized tone.
cfg.comparisonLabels = "compactV27";
...
%% ---------------- Audio files ----------------
% Same validated files used in real_instrument_resynthesis_results_V27.m,
% envelope_results_V27.m, and temporal_segmentation_results_V27.m.
audioFiles = [
...
fprintf('Original-vs-compact-resynthesis V27 batch starting.\n');
...
for iFile = 1:numel(audioFiles)
    ...
    for iVar = 1:numel(cfg.comparisonLabels)
        ...
        try
            paramsVariant = local_make_variant_params(S.params, variant);  % full compact model; no fields removed
...
save(fullfile(cfg.resultsDir, 'objective_parameter_batch_summary.mat'), ...
    ...
    'literatureBitrateTemplate', 'harmonicErrorTable', 'summaryTable', 'failureTable');
...
function paramsOut = local_make_variant_params(paramsIn, comparisonLabel)
    %#ok<INUSD>
    % This original-comparison script uses the full compact V27 model only.
    % No AM/FM/envelope fields are removed here.
    paramsOut = paramsIn;
end
...
function local_write_readme(resultsDir)
    txt = [ ...
        "Original-vs-compact-resynthesis objective + parameter results output" + newline + newline + ...
        ...
        "tables/summary_by_family.csv: family-level averages for the compact V27 model." + newline + ...
        ...
        "then summarize instrument-family trends using summary_by_family.csv and bit-rate comparisons using bitrate_summary_table.csv." + newline];
