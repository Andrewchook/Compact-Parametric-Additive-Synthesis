function S = vib_analyze_harmonics_analysis_V27(audioIn, opts)
%VIB_ANALYZE_HARMONICS_ANALYSIS_V27  Per-harmonic AM/FM vibrato analysis pipeline.
%
% Path-input patch: first input may be a numeric audio vector, string/char audio path, or [] with opts.audioPath.
%
% Clean use:
%   analysisOpts.fs = fs;
%   S = vib_analyze_harmonics_analysis_V27(x, analysisOpts);
%
% Or read directly from an audio file path:
%   S = vib_analyze_harmonics_analysis_V27("violin.wav", analysisOpts);
%
...
%   the extracted FM in Hz. Results are stored in S.params. Resynthesis is handled by vib_analyze_harmonics_resynthesize_V27.m.
%
% V14-global change:
...
arguments
    ...
    % V27: peak-anchored onset correction for plucked/sharp attacks with early low-level noise
    ...
    % V27 vibrato rejection/confidence gates used before storing compact params.
...
% ---- Estimate or set f0 ----
% V27: robust majority-vote pitch estimation. The old implementation used
...
% V27 changes the onset logic from a simple first-threshold crossing to a
...
% V27 attack end: if the main global peak is reached within the sharp-attack
...
%LOCAL_MAKE_RESYNTHESIS_PARAMS  Build the compact struct needed by V27 resynthesis.
%
% V27 keeps S.params as a playback contract only.  Analysis diagnostics,
...
params = struct();
params.version = 'V27_resynthesis_params_compact';
params.sourceFs = fs;
...
for k = 1:K
    ...
    % V27 inherits the V23 attack rejection: never fit sinusoidal vibrato
...
paramsOut = struct();
paramsOut.version = 'V27_resynthesis_params_compact';
...
paramsOut.harm = repmat(struct('k',[], 'envModel',[], 'amModel',[], 'fmModel',[], 'phaseOffsetRad',[]), K, 1);
for kk = 1:K
    hk = paramsIn.harm(kk);
    ...
    paramsOut.harm(kk).fmModel = local_compact_fit_model(local_copy_field(hk, 'fmModel', struct()));
