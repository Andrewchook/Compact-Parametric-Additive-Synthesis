function [ySynth, synthData] = vib_analyze_harmonics_resynthesize_V30(params, opts)
%VIB_ANALYZE_HARMONICS_RESYNTHESIZE_V30  Additive AM/FM resynthesis pipeline.
%
% Options patch: fs is read from opts.fs or params.sourceFs; it is not a positional input.
%
% Clean use:
%   synthOpts.fs = fs;
%   [ySynth, synthData] = vib_analyze_harmonics_resynthesize_V30(S.params, synthOpts);
%
% This function intentionally does not use the original audio. It only uses
% the parameter struct produced by vib_analyze_harmonics_analysis_V30.m.
%
% Model:
...
% fitted sinusoidal FM term. In V30, the preferred FM path reads one
% top-level H1-equivalent params.fmModel and scales its depth by harmonic
% number k. Stored vibrato start/end times gate the AM/FM
% modulation and can be overridden with opts.vibratoStartSec/EndSec.
%
% V30 change:
%   AM is optional. If params.harm(k).amModel is missing, or if
%   opts.resynthUseAM is false, the harmonic uses amGain = 1.
...
arguments
    ...
    opts.resynthUseAM (1,1) logical = true
...
if ~isfinite(sourceDur) || sourceDur <= 0
    error('V30 params must include sourceFs and sourceNumSamples, or a valid legacy sourceDurationSec.');
end
...
[useCompactFm, compactFmModel] = local_get_compact_fm_model(params);
...
for ii = 1:K
    ...
    if opts.resynthUseAM
        ...
        amModel = local_get_model(struct(), 'amModel');
        amSin = zeros(N,1);
    end
    amGain = 1 + amSin;
    ...
    if useCompactFm
        [fmModel, fmAmpHz] = local_expand_compact_fm_model(compactFmModel, k);
    else
        fmModel = local_get_model(hk, 'fmModel');
        fmAmpHz = fmModel.amp;
    end
    fmSinHz = fmAmpHz * sin(2*pi*fmModel.freqHz*t + fmModel.phaseRad);
...
synthData = struct();
synthData.version = 'V30_resynthesis';
...
% V30 stores sourceFs and sourceNumSamples; duration is derived.
sourceDur = NaN;
...
function [useCompactFm, model] = local_get_compact_fm_model(params)
%LOCAL_GET_COMPACT_FM_MODEL Return the V30 shared FM model if present.
model = struct('freqHz',0,'amp',0,'phaseRad',0,'offset',0, ...
    'refHarmonic',1,'scaleByHarmonic',true);
useCompactFm = false;
if ~isfield(params, 'fmModel') || isempty(params.fmModel) || ~isstruct(params.fmModel)
    return;
end
src = params.fmModel;
...
if ~isfinite(model.freqHz) || model.freqHz <= 0 || ~isfinite(model.amp) || model.amp <= 0
    model.freqHz = 0;
    model.amp = 0;
    model.phaseRad = 0;
    return;
...
if ~isfinite(model.refHarmonic) || model.refHarmonic <= 0
    model.refHarmonic = 1;
end
...
function [modelOut, ampHz] = local_expand_compact_fm_model(modelIn, k)
%LOCAL_EXPAND_COMPACT_FM_MODEL Convert the stored H1-equivalent FM depth
% into the actual FM depth for harmonic k.
modelOut = modelIn;
ampHz = modelIn.amp;
if isfield(modelIn, 'scaleByHarmonic') && modelIn.scaleByHarmonic
    refH = 1;
    if isfield(modelIn, 'refHarmonic') && isfinite(modelIn.refHarmonic) && modelIn.refHarmonic > 0
        refH = modelIn.refHarmonic;
    end
    ampHz = modelIn.amp * (k / refH);
end
modelOut.amp = ampHz;
end
...
% V30 preferred path: rebuild the sampled envelope from compact fit
...
t = (0:numel(y)-1).' / fs;
figure('Name','V30 Resynthesized Audio','Color','w');
plot(t, y);
grid on; xlabel('Time (s)'); ylabel('Amplitude');
title('V30 parameter-only resynthesis');
end
...
figAM = figure('Name','V30 AM Controls Per Harmonic','Color','w');
...
figFM = figure('Name','V30 FM Controls Per Harmonic','Color','w');
...
figA = figure('Name','V30 Harmonic Amplitude Controls','Color','w');
...
figF = figure('Name','V30 Harmonic Frequency Controls','Color','w');
